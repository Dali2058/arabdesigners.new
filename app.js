...
const BADGE_META = {
  owner:{img:'https://i.postimg.cc/wvxpxjDR/owner.png',label:'Owner'},
...
// Owner/Staff are display-only badges for the founder account. They never grant permissions.
const OWNER_USERNAME = 'i.ixi.';
...
function badgeList(d){
  ...
  // Owner account keeps only the Owner badge visually.
  // Other stored role/verification badges do not render for i.ixi.
  if(username===OWNER_USERNAME.toLowerCase()) return ['owner'];
...
async function home(){
  ...
  <section class="section home-marquee home-image-marquee" aria-label="Creative fields"><div class="marquee-track image-marquee-track">${Array.from({length:10},()=>`<span class="marquee-image"><img src="https://i.postimg.cc/VkHG0LyS/Asset-13-2x.png" alt="Arab Designers creative mark" loading="lazy"></span>`).join('')}</div></section>
  <section class="section home-showcase"><div class="section-head"><div><div class="section-label">WHY ARAB DESIGNERS</div><h2>Built around <span>the creator.</span></h2></div><p class="section-note">A focused space to present your work, discover people and start real conversations.</p></div><div class="home-feature-grid"><article class="home-feature-card home-feature-large"><span class="feature-index">01</span><div class="feature-icon">✦</div><h3>Profiles that feel like you.</h3><p>Your name, identity, links and work in one polished public space.</p><a href="/designers">Explore profiles ↗</a></article><article class="home-feature-card"><span class="feature-index">02</span><div class="feature-icon">◫</div><h3>Projects, not just posts.</h3><p>Publish full creative projects and let the work speak for itself.</p></article><article class="home-feature-card"><span class="feature-index">03</span><div class="feature-icon">↗</div><h3>Direct connections.</h3><p>Find a designer and start a conversation without leaving the platform.</p></article></div></section>
  <section class="section home-editorial"><div class="section-head"><div><div class="section-label">THE CREATIVE SPACE</div><h2>Made for <span>visual thinkers.</span></h2></div><p class="section-note">A visual tour of the tools, spaces and references that shape modern creative work.</p></div><div class="editorial-grid"><article class="editorial-card editorial-wide"><img src="https://images.unsplash.com/photo-1572044162444-ad60f128bdea?auto=format&fit=crop&fm=jpg&q=80&w=1600" alt="Graphic designer working with color references" loading="lazy"><div class="editorial-copy"><span>01 · VISUAL IDENTITY</span><h3>Ideas need a strong system.</h3><p>From typography to color, build a recognizable language that can travel across every surface.</p></div></article><article class="editorial-card"><img src="https://images.unsplash.com/photo-1548094990-c16ca90f1f0d?auto=format&fit=crop&fm=jpg&q=80&w=1200" alt="Minimal creative workspace" loading="lazy"><div class="editorial-copy"><span>02 · WORKSPACE</span><h3>Make room for the craft.</h3><p>The best work often starts with a focused desk, a clear brief and time to explore.</p></div></article><article class="editorial-card"><img src="https://images.unsplash.com/photo-1642884422166-256f19bcbd85?auto=format&fit=crop&fm=jpg&q=80&w=1200" alt="Digital interface and camera equipment" loading="lazy"><div class="editorial-copy"><span>03 · DIGITAL</span><h3>Design for the screen.</h3><p>UI, interaction and visual systems come together when every detail has a purpose.</p></div></article></div></section>
  <section class="section home-stats"><div class="home-stat"><strong>${formatNumber(profiles.length)}</strong><span>Designers in the network</span></div><div class="home-stat"><strong>∞</strong><span>Ideas worth showing</span></div><div class="home-stat"><strong>24/7</strong><span>Portfolio access</span></div><div class="home-stat"><strong>AR</strong><span>Creative community</span></div></section>
...
async function publishPage(){
  ...
  titleInput?.addEventListener('input',()=>document.getElementById('titleCount').textContent=`${titleInput.value.length}/120`); descInput?.addEventListener('input',()=>document.getElementById('descriptionCount').textContent=`${descInput.value.length}/400`);
  ...
  function renderStaged(){
    ...
    for(let i=0;i<stagedBlocks.length;i++){
      ...
      if(b.kind==='image' && b.layout==='half'){
        const row=[];
        while(i<stagedBlocks.length && stagedBlocks[i].kind==='image' && stagedBlocks[i].layout==='half'){
          row.push(stagedBlocks[i]); i++;
        }
        i--;
        blocksHtml+=`<div class="publish-image-row" style="margin-top:${Number(row[0].gap||0)}px">${row.map(x=>blockPreviewHtml(x)).join('')}</div>`;
      }else blocksHtml+=blockPreviewHtml(b);
    }
    ...
    const coverPicker=document.getElementById('choosePublishCoverBtn');
    if(coverPicker) coverPicker.textContent=coverFile?'▧ Change cover':'▧ Choose cover';
    blockList.innerHTML=stagedBlocks.map((b,i)=>`<div class="publish-block-row" draggable="true" data-block-index="${i}">
      <span class="publish-drag" title="Drag to reorder">⠿</span><span class="publish-block-index">${i+1}</span>
      <span class="publish-block-type">${esc(b.label)}</span><span class="publish-block-name">${esc(b.name||b.content||b.url||'')}</span>
      ${b.kind==='image'?`<select class="publish-layout" data-layout="${i}" title="Layout"><option value="full" ${b.layout!=='half'?'selected':''}>Full</option><option value="half" ${b.layout==='half'?'selected':''}>½ Side</option></select>`:''}
      <select class="publish-gap" data-gap="${i}" title="Space above"><option value="0" ${!b.gap?'selected':''}>0px</option><option value="16" ${Number(b.gap)===16?'selected':''}>16px</option><option value="32" ${Number(b.gap)===32?'selected':''}>32px</option><option value="56" ${Number(b.gap)===56?'selected':''}>56px</option><option value="80" ${Number(b.gap)===80?'selected':''}>80px</option></select>
      <button type="button" class="icon-btn danger" data-remove-block="${i}" title="Remove">✕</button>
    </div>`).join('');
    ...
    blockList.querySelectorAll('[data-layout]').forEach(s=>s.onchange=()=>{stagedBlocks[Number(s.dataset.layout)].layout=s.value;renderStaged();});
    blockList.querySelectorAll('[data-gap]').forEach(s=>s.onchange=()=>{stagedBlocks[Number(s.dataset.gap)].gap=Number(s.value);renderStaged();});
    let dragIndex=null;
    blockList.querySelectorAll('.publish-block-row').forEach(row=>{
      row.addEventListener('dragstart',()=>{dragIndex=Number(row.dataset.blockIndex);row.classList.add('dragging')});
      row.addEventListener('dragend',()=>row.classList.remove('dragging'));
      row.addEventListener('dragover',e=>e.preventDefault());
      row.addEventListener('drop',e=>{e.preventDefault();const to=Number(row.dataset.blockIndex);if(dragIndex===null||dragIndex===to)return;const [moved]=stagedBlocks.splice(dragIndex,1);stagedBlocks.splice(to,0,moved);dragIndex=null;renderStaged();});
    });
  ...
  document.getElementById('customButtonBtn').onclick=()=>{document.getElementById('toolModalTitle').textContent='Custom Button';document.getElementById('toolModalBody').innerHTML=`<label class="label">Button text</label><input class="input" id="cbText" value="View project"><label class="label">Link</label><input class="input" id="cbUrl" placeholder="https://…"><button class="btn primary full" id="saveCb">Add button</button>`;document.getElementById('toolModal').classList.add('open');document.getElementById('saveCb').onclick=()=>{const text=document.getElementById('cbText').value.trim()||'View project',url=document.getElementById('cbUrl').value.trim();if(!/^https:\/\//i.test(url)){notify('Please use an HTTPS URL.');return}stagedBlocks.push({kind:'embed',label:'Custom Button',content:text,url,gap:24});renderStaged();document.getElementById('toolModal').classList.remove('open')}};
  document.getElementById('saveDraftBtn').onclick=()=>{localStorage.setItem('ARAB_DRAFT_WORK',JSON.stringify({title:'',description:'',blocks:stagedBlocks.map(b=>({kind:b.kind,label:b.label,name:b.name,content:b.content,url:b.url,layout:b.layout||'full',gap:Number(b.gap||0)}))}));notify('Draft saved locally.');};
  const openDetails=()=>{document.getElementById('publishDetailsModal').classList.add('open');renderStaged();updatePublishPreview();};
  document.getElementById('publishNowBtn').onclick=openDetails;
  document.getElementById('choosePublishCoverBtn').onclick=()=>document.getElementById('publishCoverFile').click(); document.getElementById('changeCoverBtn').onclick=()=>document.getElementById('publishCoverFile').click();
  document.getElementById('savePublishDraft').onclick=()=>{localStorage.setItem('ARAB_DRAFT_WORK',JSON.stringify({title:titleInput.value,description:descInput.value,category:categoryInput.value,tags:publishTags,tools:toolsInput.value,blocks:stagedBlocks.map(b=>({kind:b.kind,label:b.label,name:b.name,content:b.content,url:b.url,layout:b.layout||'full',gap:Number(b.gap||0)}))}));notify('Draft saved locally.');};
...
function about(){
  shell(`<section class="about-hero about-hero-v2 about-hero-premium"><div><div class="section-label">ABOUT · ARAB DESIGNERS</div><h1>Design with<br><span>direction.</span></h1><p>Arab Designers is a focused creative network for designers, visual thinkers and people who care about the details behind the work.</p><div class="about-actions"><a class="btn primary" href="/designers">Meet the designers ↗</a><a class="text-link" href="/contact">Start a project →</a></div><div class="about-signature"><span>CREATIVE NETWORK</span><span>VISUAL IDENTITY</span><span>UI/UX</span><span>DIRECTION</span></div></div><div class="about-portrait about-portrait-v2 about-orbit-card"><img src="${EYAD}" alt="Arab Designers"><div class="portrait-overlay"></div><div class="portrait-tag">ARAB DESIGNERS<br><small>CREATIVE NETWORK · 2026</small></div><span class="orbit orbit-one"></span><span class="orbit orbit-two"></span></div></section>
  <section class="section about-manifesto"><div class="manifesto-word">MAKE</div><div><div class="section-label">OUR POINT OF VIEW</div><h2>Good design is not noise.<br><span>It is clarity.</span></h2><p>We believe a strong visual idea should be easy to recognize, difficult to forget and useful beyond the first impression.</p></div></section>
  <section class="section about-pillars"><div class="section-head"><div><div class="section-label">THE PRINCIPLES</div><h2>What we <span>care about.</span></h2></div></div><div class="about-pillar-grid"><article><b>01</b><h3>Clarity</h3><p>Every type choice, layout and interaction has a reason.</p></article><article><b>02</b><h3>Character</h3><p>Good work has a point of view. It should feel owned, not generic.</p></article><article><b>03</b><h3>Craft</h3><p>Small details create the difference between finished and memorable.</p></article><article><b>04</b><h3>People</h3><p>The platform exists to make creative talent easier to discover.</p></article></div></section>
  <section class="section about-process"><div class="section-label">HOW IT WORKS</div><div class="process-rail"><article><span>01</span><div><h3>Discover</h3><p>Explore designers by profile, identity and published work.</p></div></article><article><span>02</span><div><h3>Present</h3><p>Build a project that gives your work the space it deserves.</p></div></article><article><span>03</span><div><h3>Connect</h3><p>Contact creators directly and turn a profile visit into a conversation.</p></div></article></div></section>
  <section class="section about-visual-story"><div class="section-head"><div><div class="section-label">INSIDE THE PRACTICE</div><h2>Where ideas <span>take shape.</span></h2></div><p class="section-note">A few visual references from the world of design, interfaces, typography and creative studios.</p></div><div class="about-visual-grid"><article><img src="https://images.unsplash.com/photo-1572044162444-ad60f128bdea?auto=format&fit=crop&fm=jpg&q=80&w=1400" alt="Designer working with colors" loading="lazy"><div><span>IDENTITY</span><h3>Build a visual language.</h3><p>Strong identity work turns a collection of assets into something people can recognize instantly.</p></div></article><article><img src="https://images.unsplash.com/photo-1548094990-c16ca90f1f0d?auto=format&fit=crop&fm=jpg&q=80&w=1200" alt="Creative desk setup" loading="lazy"><div><span>PROCESS</span><h3>Make space for experiments.</h3><p>References, sketches and small iterations are part of the finished result.</p></div></article><article><img src="https://images.unsplash.com/photo-1642884422166-256f19bcbd85?auto=format&fit=crop&fm=jpg&q=80&w=1200" alt="Digital design tools" loading="lazy"><div><span>INTERACTION</span><h3>Details become experience.</h3><p>Good interfaces feel simple because the hard decisions were made before the user arrived.</p></div></article></div></section>
  <section class="section timeline-modern"><div class="section-label">THE STORY</div><div class="timeline-list"><article><b>01</b><div><h3>A place for Arab creatives</h3><p>Started from a simple idea: make great regional creative talent easier to discover.</p></div></article><article><b>02</b><div><h3>Profiles with personality</h3><p>Move beyond a list of links and give designers a proper public identity.</p></div></article><article><b>03</b><div><h3>Work at the center</h3><p>Projects, images, motion and visual systems are the heart of every profile.</p></div></article><article><b>04</b><div><h3>Always evolving</h3><p>New tools, better presentation and a stronger creative community are always in progress.</p></div></article></div></section>
  <section class="section about-final-cta"><div><div class="section-label">READY?</div><h2>Show people<br><span>what you can make.</span></h2></div><a class="btn primary xl" href="/login.html">Create your profile ↗</a></section>`);
}
function contact(){
...
async function openWorkViewer(workId){
  ...
  cloudCall('list-work-blocks',{workId:w.id}).then(r=>{
    const blocks=r.blocks||[]; let html='';
...
async function adminPage(){
  ...
  document.querySelectorAll('[data-clear-user-badges]').forEach(b=>b.onclick=async()=>{const username=b.dataset.clearUserBadges;if(username.toLowerCase()===OWNER_USERNAME.toLowerCase())return;if(!confirm(`Remove every badge from @${username}? The Owner badge is always protected.`))return;b.disabled=true;b.textContent='Removing…';try{const r=await cloudCall('clear-user-badges',{username});notify(r.message||'All badges removed');await adminPage();}catch(e){b.disabled=false;b.textContent='Remove all badges';notify(e.message||'Admin action failed')}});
  document.getElementById('clearAllBadges')?.addEventListener('click',async()=>{if(!confirm('Remove ALL badges from all users? The Owner badge for i.ixi. will stay protected.'))return;const b=document.getElementById('clearAllBadges');b.disabled=true;b.textContent='Removing…';try{const r=await cloudCall('clear-all-badges');notify(r.message||'All badges cleared. Owner kept.');await adminPage();}catch(e){b.disabled=false;b.textContent='Remove all badges · keep Owner';notify(e.message||'Admin action failed')}});
