...
function footer(){
  return `<footer class="footer"><div class="footer-main"><div><div class="footer-brand"><span class="brand-mark"><img src="${LOGO}" onerror="this.src='${FALLBACK_LOGO}'" alt=""></span><strong>Arab Designers</strong></div><p>A premium space for Arabic creatives, portfolios and visual work.</p></div><div><b>Explore</b><a href="/discover">Discover</a><a href="/designers">Designers</a><a href="/about">About</a></div><div><b>Studio</b><a href="/contact">Start a project</a><a href="/settings.html">Creator settings</a><a href="/login.html">Join community</a></div></div><div class="footer-bottom"><span>© 2026 Arab Designers</span><span>Built for people who care about the details.</span></div></footer>`;
}
...
async function profile(username){
  ...
  shell(`<section class="profile-shell">
    ...
    <div class="profile-share-card">
      <div class="share-card-media" ${banner?`style="background-image:url('${esc(banner)}')"`:''}></div>
      <div class="share-card-shade"></div><img class="share-card-avatar" src="${esc(safeImage(d.avatar))}" alt="">
      <div class="share-card-name"><strong>${esc(d.display_name||d.username)}</strong><span>@${esc(d.username)}</span></div>
      <div class="share-card-brand"><img src="${LOGO}" alt=""><span>Powered by Arab Designers</span></div>
    </div>
    <div class="profile-main"><div class="profile-heading"><img class="profile-avatar-v2" src="${esc(safeImage(d.avatar))}" alt="${esc(d.display_name||d.username)}"><div class="profile-title"><div class="verified-line"><span class="status-dot"></span> Designer profile ${d.verified?'<span class="verified-badge" title="Verified">✓</span>':''} ${d.role==='admin'?'<span class="staff-badge" title="Staff">🛡</span>':''}</div><h1>${esc(d.display_name||d.username)}</h1><p>@${esc(d.username)}</p></div><div class="profile-actions">${same?'<button class="btn" id="openWork">+ Add work</button><a class="btn" href="/settings.html">Edit profile</a>':'<a class="btn primary" href="/contact.html?designer=${encodeURIComponent(target)}">Contact designer ↗</a>'}</div></div>
...
async function openWorkEditor(username, designId=null){
  try{ await loadCloudState(); }catch(e){ console.warn('Editor opened while cloud is unavailable:', e); }
  const d=readProfiles()[username] || (me && me.username===username ? {username:me.username,display_name:me.display_name||me.username,avatar:me.avatar,banner:me.banner,discordBanner:me.banner,bio:'',designs:[],links:[]} : null);
  if(!d){ notify('Sign in with Discord before adding a project.'); return; }
  ...
  const input=document.getElementById('workFile'),drop=document.getElementById('dropzone'),preview=document.getElementById('preview'),row=document.getElementById('fileRow');let selected=null,previewUrl='';const openPicker=()=>{try{input.click()}catch(err){console.error(err);notify('Your browser blocked the file picker. Try the drop area again.')}};document.getElementById('chooseFile').onclick=e=>{e.preventDefault();e.stopPropagation();openPicker()};drop.addEventListener('click',e=>{if(e.target.closest('button'))return;openPicker()});drop.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openPicker()}});drop.tabIndex=0;
  ...
  function loadFile(file){if(!file)return;const ok=/^(image\/(png|jpeg|webp|gif)|video\/(mp4|webm|quicktime|ogg))$/i.test(file.type)||/\.(png|jpe?g|webp|gif|mp4|webm|mov|m4v)$/i.test(file.name);if(!ok){notify('Please choose an image, GIF or video.');return}if(file.size>30*1024*1024){notify('Please keep media under 30 MB.');return}if(previewUrl)URL.revokeObjectURL(previewUrl);selected=file;previewUrl=URL.createObjectURL(file);row.innerHTML=`<span>${esc(file.name)} · ${(file.size/1024/1024).toFixed(1)} MB</span><button class="icon-btn danger" type="button" id="clearSelected">Remove</button>`;document.getElementById('clearSelected').onclick=()=>{selected=null;input.value='';row.innerHTML='';if(previewUrl)URL.revokeObjectURL(previewUrl);preview.innerHTML=existing?mediaMarkup(existing,'preview-media'):'<div class="preview-empty"><span>Preview</span><small>Your work will appear here</small></div>'};preview.innerHTML=file.type.startsWith('video/')?`<video class="preview-media" src="${previewUrl}" controls muted playsinline></video>`:`<img class="preview-media" src="${previewUrl}" alt="Live preview">`}
  document.getElementById('saveWork').onclick=async()=>{const title=document.getElementById('workTitle').value.trim()||'Untitled project';const btn=document.getElementById('saveWork');btn.disabled=true;btn.textContent='Uploading…';try{if(!selected&&!existing){throw new Error('Choose a file first.')}let mediaUrl=existing?.src||'',storagePath=existing?.storagePath||'',mediaType=existing?.mediaType||'image',mediaLabel=existing?.mediaLabel||'Image';if(selected){if(!me || !sessionStorage.getItem('discord_token')) throw new Error('Sign in with Discord before uploading a project.'); const signed=await cloudCall('create-upload',{filename:selected.name,contentType:selected.type,size:selected.size});if(!window.__ARAB_SB)throw new Error('Cloud storage client is not ready');const up=await window.__ARAB_SB.storage.from(WORK_BUCKET).uploadToSignedUrl(signed.path,signed.token,selected,{contentType:selected.type,cacheControl:'31536000'});if(up.error)throw new Error(up.error.message||'Storage upload failed');storagePath=signed.path;mediaUrl=publicWorkUrl(storagePath);mediaType=selected.type.startsWith('video/')?'video':'image';mediaLabel=(selected.type==='image/gif'||/\.gif$/i.test(selected.name))?'GIF':mediaType==='video'?'Video':'Image'}await cloudCall('publish-work',{id:existing?.id||uid(),title,mediaUrl,mediaType,mediaLabel,storagePath});await refreshAfterMutation();close();notify(existing?'Project updated in the cloud':'Project published to the cloud');setTimeout(()=>profile(username),180)}catch(err){console.error(err);notify(err.message==='Failed to fetch'?'Upload service is offline. Deploy the arab-designers-api Edge Function first.':(err.message||'Could not publish this project.'))}finally{btn.disabled=false;btn.textContent=existing?'Save changes':'Publish project ↗'}};
}
...
async function handleOAuth(){
  ...
  try{const r=await fetch('https://discord.com/api/v10/users/@me',{headers:{Authorization:'Bearer '+token}});if(!r.ok)throw new Error('oauth');const u=await r.json();const avatar=avatarUrl(u);const discordBanner=bannerUrl(u);const user={id:u.id,username:u.username,display_name:u.global_name||u.username,avatar,banner:discordBanner,premium_type:u.premium_type||0,hasDiscordBanner:!!u.banner,role:u.username===ADMIN_USERNAME?'admin':'designer',verified:u.username===ADMIN_USERNAME};saveMe(user);await cloudCall('sync-profile');cloudLoaded=false;await loadCloudState(true);history.replaceState({},'',location.pathname+location.search);location.href='/home.html';return true}catch(e){console.error(e);notify(e.message||'Cloud/Discord login could not be completed.');return false}
}
