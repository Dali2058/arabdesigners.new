...
route().catch(err=>{console.error(err);if(app)app.innerHTML=`<main class="auth-page"><section class="auth-card"><div class="section-label">CLOUD SETUP</div><h1>One last connection.</h1><p>The site is ready for Supabase cloud storage. Add your Supabase URL and publishable/anon key in <code>cloud-config.js</code>, then deploy the included Edge Function.</p><a class="btn primary xl full" href="/home.html">Refresh ↗</a></section></main>`});
